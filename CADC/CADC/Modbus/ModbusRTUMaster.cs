...
namespace CADC.Modbus
{
    public class ModbusRTUMaster
    {
		...
		#endregion
		#region TimoutCnt
		private int nTimeoutCnt = 0;
		public int TimeoutCnt
		{
			get { return nTimeoutCnt; }
			set { nTimeoutCnt = value; }
		}
		#endregion
		#region Bits
        ...
        */
        ...
        public SerialPort ser = new SerialPort();
        private Thread th;
        private bool IsRunning = false;
        #endregion
        #region Event
        public event EventHandler ValueChanged;
        public event EventHandler ValueNonChaned;
        ...
        public ModbusRTUMaster(Form frm) { this.frm = frm; }
        public ModbusRTUMaster(Form frm, string Port, int Baurate) { this.frm = frm; this.PortName = Port; this.Baudrate = Baudrate; }
        ...
        */
        ...
        private void Run()
        {
                        ...
                        while (bRepeat)
                        {
                            if (w != null)
                            {
								...
								try
								{
									...
									//Debug.WriteLine(String.Format("Send Data : {0}", CommonUtil.ByteArrayToString(w.data)));
									ser.BaseStream.Flush();
								}
								catch (TimeoutException)
								{
									Debug.WriteLine("TimeoutExecption 1");
								}
                                #endregion
								...
								if (w.data[0] != 0 && w.data[1] != 0x06 && w.data[1] != 0x10)//Broadcast
                                {
                                    #region Read
                                    while (nRecv < w.ResponseByteCount)
                                    {
										...
										{
											Debug.WriteLine("TimeoutExecption 2");
											nTimeoutCnt++;
										}
                                                    ...
                                                    case ModbusFunction.BITREAD_F2:
                                                        {
                                                            ...
                                                            if (bChanged && ValueChanged != null)
                                                            {
                                                                ...
                                                                if (frm != null) frm.Invoke(ValueNonChaned, new object[] { this, null });
                                                            else ValueNonChaned.Invoke(this, null);
                                                            {
                                                            }
                                                            #endregion
                                                            #region Remark
            ...
            {
				MessageBox.Show("통신포트가 발견되지 않았습니다. 통신포트를 다시 선택하시거나 포트가 연결되었는지 확인하시기 바랍니다. CODE : 41 오류");
			}
            catch (UnauthorizedAccessException)
            {
                MessageBox.Show("통신포트에 문제가 발생하였습니다. 이미 연결된 장치가 있는지 확인하시기 바랍니다. CODE : 42 오류");
			}
			catch (ArgumentException)
            {
                MessageBox.Show("통신포트가 발견되지 않았습니다. 통신포트를 다시 선택하시거나 포트가 연결되었는지 확인하시기 바랍니다. CODE : 43 오류");
            }
            catch (InvalidOperationException)
            {
                MessageBox.Show("통신포트가 발견되지 않았습니다. 통신포트를 다시 선택하시거나 포트가 연결되었는지 확인하시기 바랍니다. CODE : 44 오류");
            }
            catch (IOException)
            {
                MessageBox.Show("통신포트가 발견되지 않았습니다. 통신포트를 다시 선택하시거나 포트가 연결되었는지 확인하시기 바랍니다. CODE : 45 오류");
            }
    ...
    public class ModbusAddress
    {
        ...
        internal ModbusRTUMaster.Work GetWork()
        {
            ...
            if (AddressType == ModbusAddressType.BIT)
            {
                ...
                byte crcHi = 0xff, crcLo = 0xff;
                ModbusRTUMaster.GetCRC(lst, 0, lst.Count, ref crcHi, ref crcLo);
                lst.Add(crcHi);
                lst.Add(crcLo);
            ...
            ModbusRTUMaster.Work ret = new ModbusRTUMaster.Work(lst.ToArray(), nResCount);
            return ret;
        }

        internal ModbusRTUMaster.Work GetTcpWork()
        {
            List<byte> lst = new List<byte>();
            int nResCount = 0;
